...
void UAN_XBMeleeHit::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
    ...
    UE_LOG(LogXBCombat, Log, TEXT("[AN_XBMeleeHit] 执行单次伤害检测 - 攻击者: %s, 伤害: %.1f"), 
    ...
    if (HitResults.Num() > 0)
    {
        ...
        UE_LOG(LogXBCombat, Log, TEXT("[AN_XBMeleeHit] 命中 %d 个目标"), HitResults.Num());
    }
    else
    {
...
TArray<FHitResult> UAN_XBMeleeHit::PerformHitDetection(USkeletalMeshComponent* MeshComp)
{
    ...
    if (!HitConfig.StartSocketName.IsNone() && MeshComp->DoesSocketExist(HitConfig.StartSocketName))
    {
        FTransform StartTransform = MeshComp->GetSocketTransform(HitConfig.StartSocketName, ERelativeTransformSpace::RTS_World);
        StartLocation = StartTransform.GetLocation() + StartTransform.GetRotation().RotateVector(HitConfig.StartLocationOffset);
    }
    else
    {
        StartLocation = MeshComp->GetComponentLocation() + HitConfig.StartLocationOffset;
    }

    if (!HitConfig.EndSocketName.IsNone() && MeshComp->DoesSocketExist(HitConfig.EndSocketName))
    {
        FTransform EndTransform = MeshComp->GetSocketTransform(HitConfig.EndSocketName, ERelativeTransformSpace::RTS_World);
        EndLocation = EndTransform.GetLocation() + EndTransform.GetRotation().RotateVector(HitConfig.EndLocationOffset);
    }
    else
    {
        EndLocation = StartLocation + FVector(0, 0, 100.0f) + HitConfig.EndLocationOffset;
    }
    ...
    bool bHit = UKismetSystemLibrary::SphereTraceMultiForObjects(
        ...
        false,
        IgnoreActors,
        HitConfig.bEnableDebugDraw ? EDrawDebugTrace::ForDuration : EDrawDebugTrace::None,
        HitResults,
        true,
        FLinearColor::Red,
        FLinearColor::Green,
    ...
    if (HitConfig.bEnableDebugDraw)
    {
        ...
        {
            DrawDebugSphere(World, StartLocation, ScaledRadius, 12, FColor::Blue, false, HitConfig.DebugDrawDuration);
            DrawDebugSphere(World, EndLocation, ScaledRadius, 12, FColor::Cyan, false, HitConfig.DebugDrawDuration);
            DrawDebugLine(World, StartLocation, EndLocation, bHit ? FColor::Red : FColor::Green, false, HitConfig.DebugDrawDuration, 0, 3.0f);
            
            FVector TextLocation = (StartLocation + EndLocation) * 0.5f + FVector(0, 0, ScaledRadius + 20.0f);
            DrawDebugString(World, TextLocation,
                FString::Printf(TEXT("单次检测 缩放: %.2fx, 命中: %d"), OwnerScale, HitResults.Num()),
...
bool UAN_XBMeleeHit::ShouldDamageTarget(AActor* OwnerActor, AActor* TargetActor) const
{
    ...
    return UXBBlueprintFunctionLibrary::AreFactionsHostile(OwnerFaction, TargetFaction);
}
...
void UAN_XBMeleeHit::ApplyDamageToTargets(const TArray<FHitResult>& HitResults, AActor* OwnerActor)
{
    if (!OwnerActor)
    {
        return;
    ...
    for (const FHitResult& Hit : HitResults)
    {
        ...
        if (DamageEffectClass && SourceASC)
        {
            ...
            if (SpecHandle.IsValid())
            {
                ...
                UE_LOG(LogXBCombat, Log, TEXT("[AN_XBMeleeHit] 命中将领: %s, 伤害: %.1f (GAS)"), 
        ...
        else if (TargetASC)
        {
            ...
            UE_LOG(LogXBCombat, Log, TEXT("[AN_XBMeleeHit] 命中: %s, 伤害: %.1f (直接属性)"), 
                *HitActor->GetName(), AttackDamage);
        }
    }
}
