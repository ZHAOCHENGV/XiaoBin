...
AXBCharacterBase::AXBCharacterBase() {
  ...
  CombatComponent =
      CreateDefaultSubobject<UXBCombatComponent>(TEXT("CombatComponent"));

  HealthBarComponent = CreateDefaultSubobject<UXBWorldHealthBarComponent>(
      TEXT("HealthBarComponent"));
  HealthBarComponent->SetupAttachment(RootComponent);

  MagnetFieldComponent = CreateDefaultSubobject<UXBMagnetFieldComponent>(
      TEXT("MagnetFieldComponent"));
  MagnetFieldComponent->SetupAttachment(RootComponent);

  FormationComponent = CreateDefaultSubobject<UXBFormationComponent>(
      TEXT("SoldierFormationComponent"));
 ...
 * @brief  角色初始化入口
 * @return 无
 * @note   详细流程分析: 注册感知 -> 初始化组件 -> 初始化主将数据 ->
 * 进入运行逻辑 性能注意: 初始化仅在 BeginPlay 执行，避免运行期重复调用
 */
 ...
 * @brief  初始化主将数据
 * @return 无
 * @note   详细流程分析: 读取外部配置 -> 同步基础参数 ->
 * 根据配置行名初始化数据表 性能注意: 仅在 BeginPlay 阶段调用一次
 */
 ...
 */
bool AXBCharacterBase::GetExternalInitConfig(
...
void AXBCharacterBase::RefreshRecruitedSoldiersAfterLeaderInit() {
  ...
  UE_LOG(LogXBCharacter, Log,
         TEXT("主将 %s 数据初始化完成，刷新已招募士兵跟随状态，数量: %d"),
         *GetName(), Soldiers.Num());

  // 🔧 修改 - 先确保编队槽位覆盖当前士兵数量，避免槽位缺失导致目标位置无效
  if (FormationComponent &&
      FormationComponent->GetFormationSlots().Num() < Soldiers.Num()) {
  ...
  for (int32 Index = 0; Index < Soldiers.Num(); ++Index) {
    ...
    Soldier->RefreshFollowingAfterLeaderInit(this,
...
void AXBCharacterBase::OnMagnetFieldActorEntered(AActor *EnteredActor) {
  ...
  UE_LOG(LogXBRecruit, Log, TEXT("%s: Actor 进入磁场: %s"), *GetName(),
         *EnteredActor->GetName());
}
...
void AXBCharacterBase::InitializeFromDataTable(UDataTable *DataTable,
                                               FName RowName) {
  if (!DataTable || RowName.IsNone()) {
    return;
  }

  FXBLeaderTableRow *LeaderRow = DataTable->FindRow<FXBLeaderTableRow>(
      RowName, TEXT("AXBCharacterBase::InitializeFromDataTable"));
  if (!LeaderRow) {
    return;
  }
  ...
  GrowthConfigCache.DamageMultiplierPerSoldier =
      LeaderRow->DamageMultiplierPerSoldier;
  ...
  UE_LOG(
      LogXBCharacter, Log,
      TEXT("主将 %s 视觉配置加载完成: Mesh=%s, AnimClass=%s, DeathMontage=%s"),
      *GetName(),
...
void AXBCharacterBase::ApplyInitialAttributes() {
  ...
  const UXBAttributeSet *LocalAttributeSet =
      AbilitySystemComponent->GetSet<UXBAttributeSet>();
  if (!LocalAttributeSet) {
    return;
  }

  // 🔧 修改 - 生命值需要同时应用基础值与倍率，确保初始化即反映配置
  const float EffectiveMaxHealth = FMath::Max(
      0.01f, CachedLeaderData.MaxHealth * CachedLeaderData.HealthMultiplier);
  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetMaxHealthAttribute(), EffectiveMaxHealth);
  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetHealthAttribute(), EffectiveMaxHealth);
  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetHealthMultiplierAttribute(),
      CachedLeaderData.HealthMultiplier);
  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetDamageMultiplierAttribute(),
      CachedLeaderData.DamageMultiplier);
  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetMoveSpeedAttribute(), CachedLeaderData.MoveSpeed);
  AbilitySystemComponent->SetNumericAttributeBase(
 ...
 * @return 无
 * @note   详细流程分析: 计算缺失数量 -> 预生成编队槽位 -> 按槽位位置生成士兵 ->
 * 写入队列槽位并完成招募
 */
void AXBCharacterBase::SpawnInitialSoldiers(int32 DesiredCount) {
  ...
  UXBSoldierPoolSubsystem *PoolSubsystem =
  ...
  for (int32 i = 0; i < MissingCount; ++i) {
    ...
    if (FormationComponent) {
      ...
      const FVector Offset =
          FVector(FMath::Cos(FMath::DegreesToRadians(Angle)) * Distance,
                  FMath::Sin(FMath::DegreesToRadians(Angle)) * Distance, 0.0f);
    ...
    if (PoolSubsystem) {
      Soldier =
          PoolSubsystem->AcquireSoldier(SpawnLocation, FRotator::ZeroRotator);
    }
...
void AXBCharacterBase::TriggerSprint() {
  ...
  bAutoSprintMove = GetLastMovementInputVector().IsNearlyZero() &&
  ...
  if (SprintDuration > 0.0f) {
    ...
    GetWorldTimerManager().SetTimer(SprintDurationTimerHandle, this,
 ...
 */
bool AXBCharacterBase::GetLastAttackedEnemyFaction(
...
void AXBCharacterBase::UpdateSprint(float DeltaTime) {
  ...
  if (!FMath::IsNearlyEqual(CurrentSpeed, TargetMoveSpeed, 1.0f)) {
    float NewSpeed = FMath::FInterpTo(CurrentSpeed, TargetMoveSpeed, DeltaTime,
                                      SpeedInterpRate);
...
bool AXBCharacterBase::IsHostileTo(const AXBCharacterBase *Other) const {
  ...
  return UXBBlueprintFunctionLibrary::AreFactionsHostile(Faction,
                                                         Other->Faction);
}
...
bool AXBCharacterBase::IsFriendlyTo(const AXBCharacterBase *Other) const {
  ...
  return UXBBlueprintFunctionLibrary::AreFactionsFriendly(Faction,
                                                          Other->Faction);
}

// ==================== 士兵管理实现 ====================

bool AXBCharacterBase::Internal_AddSoldierToArray(
...
bool AXBCharacterBase::Internal_RemoveSoldierFromArray(
    AXBSoldierCharacter *Soldier) {
  if (!Soldier) {
    return false;
  }
...
void AXBCharacterBase::AddSoldier(AXBSoldierCharacter *Soldier) {
  if (bIsDead) {
    UE_LOG(LogXBCharacter, Warning, TEXT("%s: 角色已死亡，无法添加士兵"),
           *GetName());
    return;
  }
  ...
  if (ExistingIndex != INDEX_NONE) {
    UE_LOG(LogXBSoldier, Warning,
           TEXT("%s: 士兵 %s 已在队伍中（索引: %d），同步槽位"), *GetName(),
           *Soldier->GetName(), ExistingIndex);
  ...
  UE_LOG(LogXBSoldier, Log,
         TEXT("%s: 士兵 %s 添加成功，分配槽位: %d, 当前数量: %d"), *GetName(),
         *Soldier->GetName(), SlotIndex, Soldiers.Num());
...
void AXBCharacterBase::UpdateDamageMultiplier() {
  ...
  const float AdditionalMultiplier =
      Soldiers.Num() * GrowthConfigCache.DamageMultiplierPerSoldier;

  // 🔧 修改 - 取消伤害倍率上限，确保士兵加成全部生效
  const float NewMultiplier = BaseDamageMultiplier + AdditionalMultiplier;

  AbilitySystemComponent->SetNumericAttributeBase(
      UXBAttributeSet::GetDamageMultiplierAttribute(), NewMultiplier);
}
...
float AXBCharacterBase::GetCurrentScale() const {
  if (AbilitySystemComponent) {
    return AbilitySystemComponent->GetNumericAttribute(
        UXBAttributeSet::GetScaleAttribute());
  }
  return BaseScale;
}
...
float AXBCharacterBase::GetScaledAttackRange() const {
  ...
  return BaseAttackRange * CurrentScale *
         GrowthConfigCache.AttackRangeScaleMultiplier;
}
...
void AXBCharacterBase::BindCombatEvents() {
  if (CombatComponent) {
    CombatComponent->OnAttackStateChanged.AddDynamic(
        this, &AXBCharacterBase::OnCombatAttackStateChanged);
  }
}

void AXBCharacterBase::UpdateLeaderScale() {
  const float AdditionalScale =
      Soldiers.Num() * GrowthConfigCache.ScalePerSoldier;
  const float NewScale =
      FMath::Min(BaseScale + AdditionalScale, GrowthConfigCache.MaxScale);
...
void AXBCharacterBase::SmoothLeaderScale(float DeltaTime) {
  ...
  const float NewScale = InterpSpeed > 0.0f
                             ? FMath::FInterpTo(CurrentScale, TargetLeaderScale,
...
void AXBCharacterBase::ApplyLeaderScale(float NewScale) {
  ...
  if (UCapsuleComponent *Capsule = GetCapsuleComponent()) {
    ...
    if (!FMath::IsNearlyZero(HeightDelta)) {
      const FVector AdjustedLocation =
          GetActorLocation() + FVector(0.0f, 0.0f, HeightDelta);
  ...
  if (AbilitySystemComponent) {
    AbilitySystemComponent->SetNumericAttributeBase(
        UXBAttributeSet::GetScaleAttribute(), NewScale);
  }

  if (CombatComponent && GrowthConfigCache.bEnableAttackRangeScaling) {
    const float RangeScale =
        NewScale * GrowthConfigCache.AttackRangeScaleMultiplier;
...
void AXBCharacterBase::AddHealthWithOverflow(float HealthToAdd) {
  ...
  float CurrentHealth = AbilitySystemComponent->GetNumericAttribute(
      UXBAttributeSet::GetHealthAttribute());
  float CurrentMaxHealth = AbilitySystemComponent->GetNumericAttribute(
      UXBAttributeSet::GetMaxHealthAttribute());

  float NewHealth = CurrentHealth + HealthToAdd;

  if (NewHealth > CurrentMaxHealth) {
    AbilitySystemComponent->SetNumericAttributeBase(
        UXBAttributeSet::GetMaxHealthAttribute(), NewHealth);
    AbilitySystemComponent->SetNumericAttributeBase(
        UXBAttributeSet::GetHealthAttribute(), NewHealth);
  } else {
    AbilitySystemComponent->SetNumericAttributeBase(
        UXBAttributeSet::GetHealthAttribute(), NewHealth);
  }
...
void AXBCharacterBase::UpdateSkillEffectScaling() {
  ...
  float CurrentScale = GetCurrentScale();
  float EffectScale =
      CurrentScale * GrowthConfigCache.SkillEffectScaleMultiplier;
...
void AXBCharacterBase::UpdateAttackRangeScaling() {
  ...
  float CurrentScale = GetCurrentScale();
  float ScaledRange = BaseAttackRange * CurrentScale *
                      GrowthConfigCache.AttackRangeScaleMultiplier;
}
 ...
 *         性能/架构注意事项: 复用 EnterCombat
 * 统一管理战斗计时与士兵状态，避免重复逻辑
 */
void AXBCharacterBase::NotifyAttackStarted() {
  ...
  UE_LOG(LogXBCombat, Verbose, TEXT("主将 %s 开始攻击，已触发战斗状态"),
         *GetName());
}
...
void AXBCharacterBase::EnterCombat() {
  ...
  if (bIsInCombat) {
    ...
    // 🔧 修复 -
    // 战斗中二次触发时同步士兵状态，但仅在主将已锁敌时才通知士兵进入战斗
    // 说明：避免玩家释放技能但未命中敌人时士兵错误进入战斗
    if (LastAttackedEnemyLeader.IsValid() &&
        !LastAttackedEnemyLeader->IsDead()) {
        ...
        if (Soldier && Soldier->GetSoldierState() != EXBSoldierState::Dead) {
          if (Soldier->GetSoldierState() != EXBSoldierState::Combat) {
            ...
            UE_LOG(LogXBCombat, Verbose,
                   TEXT("将领 %s 同步士兵 %s 再次进入战斗"), *GetName(),
                   *Soldier->GetName());
          }
        }
      }
...
void AXBCharacterBase::DisengageFromCombat() {
  ...
  if (bSprintWhenDisengaging) {
    ...
    GetWorldTimerManager().SetTimer(DisengageSprintTimerHandle, this,
                                    &AXBCharacterBase::StopSprint,
                                    DisengageSprintDuration, false);
  }
  ...
  GetWorldTimerManager().SetTimer(
      TempHandle, [this]() { SetSoldiersEscaping(false); },
      DisengageSprintDuration, false);
}

void AXBCharacterBase::OnCombatTimeout() { ExitCombat(); }
...
void AXBCharacterBase::ScheduleNoEnemyDisengage() {
  ...
  GetWorldTimerManager().SetTimer(NoEnemyDisengageHandle, this,
                                  &AXBCharacterBase::ExitCombat,
                                  NoEnemyDisengageDelay, false);
}
...
void AXBCharacterBase::OnAttackHit(AActor *HitTarget) {
  ...
  if (TargetLeader && IsHostileTo(TargetLeader)) {
    ...
    UE_LOG(LogXBCombat, Log, TEXT("?? %s ?????? %s??????????????"), *GetName(),
  ...
  if (TargetSoldier && UXBBlueprintFunctionLibrary::AreFactionsHostile(
                           Faction, TargetSoldier->GetFaction())) {
    // 🔧 修改 - 草丛隐身目标不可被命中
    ...
    if (AXBCharacterBase *TargetSoldierLeader =
            TargetSoldier->GetLeaderCharacter()) {
 ...
 * 详细流程: 校验主将 -> 收集存活士兵 -> 收集存活敌兵 -> 计算负载与距离 ->
 * 分配目标并广播 注意事项: 仅分配存活目标，避免无效引用
 */
void AXBCharacterBase::AssignTargetsToSoldiers(AXBCharacterBase *EnemyLeader) {
  ...
  // 获取敌方士兵数组
  const TArray<AXBSoldierCharacter *> &EnemySoldiers =
      EnemyLeader->GetSoldiers();
  ...
  for (AXBSoldierCharacter *EnemySoldier : EnemySoldiers) {
    // 过滤无效或死亡士兵
    if (EnemySoldier &&
        EnemySoldier->GetSoldierState() != EXBSoldierState::Dead) {
  ...
  for (AXBSoldierCharacter *Soldier : AliveSoldiers) {
    ...
    if (AXBSoldierCharacter *TargetSoldier =
            Cast<AXBSoldierCharacter>(Soldier->CurrentAttackTarget.Get())) {
      // 记录该敌兵被锁定次数
  ...
  for (AXBSoldierCharacter *Soldier : AliveSoldiers) {
    ...
    for (AXBSoldierCharacter *EnemySoldier : AliveEnemySoldiers) {
      ...
      // 距离评分：近战优先近，弓手优先远
      const float Distance = FVector::Dist2D(Soldier->GetActorLocation(),
                                             EnemySoldier->GetActorLocation());
      const bool bIsArcher =
          (Soldier->GetSoldierType() == EXBSoldierType::Archer);
      const float Score = bIsArcher ? Distance : -Distance;

      // 比较负载与距离评分
      if (CurrentCount < BestCount ||
          (CurrentCount == BestCount && Score > BestScore)) {
 ...
 * 注意事项: 目标仅能是敌方主将或其士兵
 */
AActor *AXBCharacterBase::AssignTargetToSoldier(
    AXBSoldierCharacter *RequestingSoldier) {
  // 校验申请者
  ...
  for (AXBSoldierCharacter *EnemySoldier : AliveEnemySoldiers) {
    ...
    const float Distance =
        FVector::Dist2D(RequestingSoldier->GetActorLocation(),
                        EnemySoldier->GetActorLocation());
    const bool bIsArcher =
        (RequestingSoldier->GetSoldierType() == EXBSoldierType::Archer);
    const float Score = bIsArcher ? Distance : -Distance;
  ...
  // 返回最终目标（敌方士兵或敌方主将）
  return BestTarget ? static_cast<AActor *>(BestTarget)
                    : static_cast<AActor *>(EnemyLeader);
}
  ...
  for (AXBSoldierCharacter *Soldier : Soldiers) {
    if (Soldier && Soldier->GetSoldierState() != EXBSoldierState::Dead) {
      ...
      if (AXBSoldierAIController *SoldierAI =
              Cast<AXBSoldierAIController>(Soldier->GetController())) {
        SoldierAI->StopBehaviorTreeLogic();
        SoldierAI->StopMovement();
      } else if (AAIController *AICtrl =
                     Cast<AAIController>(Soldier->GetController())) {
        AICtrl->StopMovement();
      }
    }
  }
...
void AXBCharacterBase::SetHiddenInBush(bool bEnableHidden) {
  ...
  // 🔧 修复 - 进入草丛时主将主动清除战斗状态和目标
  // 说明：这比士兵被动检测更及时且彻底
  if (bEnableHidden) {
    LastAttackedEnemyLeader = nullptr;
    bHasEnemiesInCombat = false;
    ExitCombat();
    UE_LOG(LogXBCharacter, Log, TEXT("主将 %s 进入草丛，清除战斗状态和目标"),
           *GetName());

    // 🔧 修复 - 通知所有锁定了自己的敌方主将清除目标
    // 说明：当主将进入草丛时，锁定它的敌方主将应该丢失目标
    UWorld *World = GetWorld();
    if (World) {
      TArray<AActor *> AllLeaders;
      UGameplayStatics::GetAllActorsOfClass(
          World, AXBCharacterBase::StaticClass(), AllLeaders);

      for (AActor *Actor : AllLeaders) {
        AXBCharacterBase *OtherLeader = Cast<AXBCharacterBase>(Actor);
        if (OtherLeader && OtherLeader != this && !OtherLeader->IsDead()) {
          // 检查该主将是否锁定了自己
          if (OtherLeader->GetLastAttackedEnemyLeader() == this) {
            // 清除该主将的目标并退出战斗
            OtherLeader->LastAttackedEnemyLeader = nullptr;
            OtherLeader->bHasEnemiesInCombat = false;
            OtherLeader->ExitCombat();

            UE_LOG(LogXBCharacter, Log,
                   TEXT("主将 %s 因目标 %s 进入草丛而清除目标并退出战斗"),
                   *OtherLeader->GetName(), *GetName());
          }
        }
      }
    }
  ...
  UE_LOG(LogXBCharacter, Log, TEXT("主将 %s 草丛隐身状态=%s"), *GetName(),
...
void AXBCharacterBase::HandleDeath() {
  ...
  if (bKillSoldiersOnDeath) {
    ...
    for (AXBSoldierCharacter *Soldier : Soldiers) {
      if (Soldier && IsValid(Soldier) &&
          Soldier->GetSoldierState() != EXBSoldierState::Dead) {
        // 解除主将绑定
      ...
      if (UAnimInstance *AnimInstance = MeshComp->GetAnimInstance()) {
        ...
        if (Duration > 0.0f) {
          ...
          if (!bDelayAfterMontage) {
            GetWorldTimerManager().SetTimer(
                DeathDestroyTimerHandle, this,
                &AXBCharacterBase::OnDestroyTimerExpired, DeathDestroyDelay,
                false);
          }
        }
      }
...
void AXBCharacterBase::SpawnDroppedSoldiers() {
  ...
  if (LastDamageInstigator.IsValid()) {
    ...
    if (AXBCharacterBase *InstigatorLeader =
            Cast<AXBCharacterBase>(LastDamageInstigator.Get())) {
      TargetLeader = InstigatorLeader;
    } else if (AXBSoldierCharacter *InstigatorSoldier =
                   Cast<AXBSoldierCharacter>(LastDamageInstigator.Get())) {
      TargetLeader = InstigatorSoldier->GetLeaderCharacter();
      UE_LOG(LogXBCharacter, Log,
             TEXT("掉落士兵：击杀者为士兵 %s，归属主将=%s"),
  ...
  if (DropSoldierClass) {
    const AXBSoldierCharacter *SoldierCDO =
        DropSoldierClass->GetDefaultObject<AXBSoldierCharacter>();
    if (SoldierCDO && SoldierCDO->GetCapsuleComponent()) {
      DropCapsuleHalfHeight =
          SoldierCDO->GetCapsuleComponent()->GetScaledCapsuleHalfHeight();
    }
  }

  UE_LOG(LogXBCharacter, Log,
         TEXT("将领 %s 死亡，生成 %d 个掉落士兵，目标将领: %s"), *GetName(),
  ...
  for (int32 i = 0; i < SoldierDropConfig.DropCount; ++i) {
    ...
    float Distance =
        FMath::RandRange(ArcConfig.MinDropDistance, ArcConfig.MaxDropDistance);

    FVector Direction =
        FRotator(0.0f, Angle, 0.0f).RotateVector(FVector::ForwardVector);
    FVector TargetLocation = SpawnOrigin + Direction * Distance;

    // 地面检测
    FHitResult HitResult;
    FVector TraceStart =
        FVector(TargetLocation.X, TargetLocation.Y,
                SpawnOrigin.Z + ArcConfig.GroundTraceUpDistance);
    FVector TraceEnd =
        FVector(TargetLocation.X, TargetLocation.Y,
                SpawnOrigin.Z - ArcConfig.GroundTraceDownDistance);
    ...
    bool bHit = World->LineTraceSingleByChannel(
        ...
        ECC_Visibility, // 🔧 修复 -
                        // 使用Visibility通道，默认Pawn不阻挡Visibility
        QueryParams);
    ...
    if (bHit) {
      ...
      TargetLocation =
          HitResult.Location +
          FVector(0.0f, 0.0f,
                  DropCapsuleHalfHeight + ArcConfig.LandingExtraZOffset);
    } else {
    ...
    if (PoolSubsystem && PoolSubsystem->HasAvailableSoldier()) {
      DroppedSoldier =
          PoolSubsystem->AcquireSoldier(SpawnOrigin, FRotator::ZeroRotator);
    }
    ...
    if (!DroppedSoldier) {
      ...
      DroppedSoldier = World->SpawnActor<AXBSoldierCharacter>(
          DropSoldierClass, SpawnOrigin, FRotator::ZeroRotator, SpawnParams);
      ...
      if (DroppedSoldier) {
        ...
        if (UCapsuleComponent *Capsule =
                DroppedSoldier->GetCapsuleComponent()) {
          Capsule->SetCollisionEnabled(ECollisionEnabled::NoCollision);
        }

        // 完成构造（触发 BeginPlay）
        DroppedSoldier->FinishSpawning(
    ...
    if (DroppedSoldier) {
      ...
      if (DropSoldierDataTable && !DropSoldierRowName.IsNone()) {
        DroppedSoldier->FullInitialize(DropSoldierDataTable, DropSoldierRowName,
                                       DropFaction);
      }

      // 启动抛物线飞行
      DroppedSoldier->StartDropFlight(SpawnOrigin, TargetLocation, ArcConfig,
                                      TargetLeader);

      UE_LOG(LogXBCharacter, Log, TEXT("掉落士兵 [%d] %s 开始飞行"), i,
             *DroppedSoldier->GetName());
    }
  }
}
 ...
 */
void AXBCharacterBase::StartDeathFade() {
  if (!bEnableDeathFade) {
    return;
  }
  ...
  GetWorldTimerManager().SetTimer(DeathFadeTimerHandle, this,
                                  &AXBCharacterBase::UpdateDeathFade,
                                  UpdateInterval, true, DeathFadeDelay);

  UE_LOG(LogXBCharacter, Verbose,
         TEXT("角色 %s: 开始死亡渐隐，延迟=%.2f秒，时长=%.2f秒"), *GetName(),
 ...
 */
void AXBCharacterBase::UpdateDeathFade() {
  USkeletalMeshComponent *MeshComp = GetMesh();
  if (!MeshComp) {
    GetWorldTimerManager().ClearTimer(DeathFadeTimerHandle);
    return;
  ...
  if (DeathFadeProgress >= 1.0f) {
    ...
    UE_LOG(LogXBCharacter, Verbose, TEXT("角色 %s: 死亡渐隐完成"), *GetName());
    return;
  }
...
void AXBCharacterBase::OnDeathMontageEnded(UAnimMontage *Montage,
                                           bool bInterrupted) {
  if (bDelayAfterMontage) {
    GetWorldTimerManager().SetTimer(DeathDestroyTimerHandle, this,
                                    &AXBCharacterBase::OnDestroyTimerExpired,
  ...
  for (AXBSoldierCharacter *Soldier : Soldiers) {
    if (Soldier && IsValid(Soldier)) {
      if (Soldier->GetSoldierState() != EXBSoldierState::Dead) {
        // Soldier->TakeSoldierDamage(Soldier->GetCurrentHealth() + 100.0f,
        // this);
        Soldier->HandleDeath();
      }
    }
