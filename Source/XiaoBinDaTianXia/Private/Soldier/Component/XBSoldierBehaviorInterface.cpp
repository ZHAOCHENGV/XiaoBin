...
void UXBSoldierBehaviorInterface::TickComponent(
 ...
 * @return 是否为可识别的战斗单位
 * @note   详细流程分析: 判定目标类型 -> 若为士兵则优先取主将阵营 ->
 * 输出阵营/主将 性能/架构注意事项: 该方法仅做轻量级类型判断，避免重复逻辑散落
 */
bool UXBSoldierBehaviorInterface::ResolveTargetFaction(
    AActor *Target, EXBFaction &OutFaction,
    AXBCharacterBase *&OutLeaderOwner) const {
  // 初始化输出，避免上层使用脏数据
  ...
  if (AXBSoldierCharacter *TargetSoldier = Cast<AXBSoldierCharacter>(Target)) {
    ...
    OutFaction = OutLeaderOwner ? OutLeaderOwner->GetFaction()
                                : TargetSoldier->GetFaction();
    return true;
  }
...
bool UXBSoldierBehaviorInterface::IsTargetValid(AActor *Target) const {
  ...
  AXBCharacterBase *TargetLeaderOwner =
      OwnerLeader->GetLastAttackedEnemyLeader();
  ...
  if (AXBSoldierCharacter *TargetSoldier = Cast<AXBSoldierCharacter>(Target)) {
    ...
    return UXBBlueprintFunctionLibrary::AreFactionsHostile(
        Soldier->GetFaction(), TargetFaction);
  }
  ...
  if (AXBCharacterBase *TargetLeader = Cast<AXBCharacterBase>(Target)) {
    ...
    // 🔧 修复 BUG1 - 目标主将在草丛中隐身，视为无效目标
    if (TargetLeader->IsHiddenInBush()) {
      return false;
    }
    return UXBBlueprintFunctionLibrary::AreFactionsHostile(
        Soldier->GetFaction(), TargetLeader->GetFaction());
  }
...
EXBBehaviorResult UXBSoldierBehaviorInterface::ExecuteAttack(AActor *Target) {
  ...
  if (!PlayAttackMontage()) {
    UE_LOG(LogXBCombat, Warning, TEXT("士兵 %s 攻击失败：未能播放攻击蒙太奇"),
           *Soldier->GetName());
    return EXBBehaviorResult::Failed;
  }
  ...
  if (Soldier->GetSoldierType() == EXBSoldierType::Archer) {
    UE_LOG(LogXBCombat, Verbose, TEXT("弓手攻击不走近战Tag: %s"),
           *Soldier->GetName());
  }
...
void UXBSoldierBehaviorInterface::ApplyDamageToTarget(AActor *Target,
                                                      float Damage) {
  if (!Target) {
    return;
  }
  ...
  if (AXBSoldierCharacter *TargetSoldier = Cast<AXBSoldierCharacter>(Target)) {
    ...
    if (!UXBBlueprintFunctionLibrary::AreFactionsHostile(Soldier->GetFaction(),
                                                         TargetFaction)) {
      return;
    }
  ...
  // 对将领应用伤害（通过 GAS）
    ...
    if (!UXBBlueprintFunctionLibrary::AreFactionsHostile(
            Soldier->GetFaction(), TargetLeader->GetFaction())) {
      return;
    }
    // TODO: 通过 GAS 应用伤害
...
EXBBehaviorResult
UXBSoldierBehaviorInterface::MoveToLocation(const FVector &TargetLocation,
  ...
  EPathFollowingRequestResult::Type Result = AIController->MoveToLocation(
      TargetLocation, AcceptanceRadius, true, true, true, true);
...
EXBBehaviorResult
UXBSoldierBehaviorInterface::MoveToActor(AActor *Target,
                                         float AcceptanceRadius) {
  AXBSoldierCharacter *Soldier = GetOwnerSoldier();
  if (!Soldier || !Target) {
    return EXBBehaviorResult::Failed;
  ...
  float Distance =
      FVector::Dist(Soldier->GetActorLocation(), Target->GetActorLocation());
  ...
  if (UCharacterMovementComponent *MoveComp = Soldier->GetCharacterMovement()) {
    if (Distance <= Soldier->GetAttackRange()) {
      ...
      // 移动阶段：提高避让权重，更好地绕开障碍
      MoveComp->AvoidanceWeight =
          FMath::Max(0.5f, Soldier->GetAvoidanceWeight());
    }
  }
  ...
  float RandomDistance =
      RandomStream.FRandRange(50.0f, 150.0f); // 50-150单位的距离偏移

  // 计算偏移后的位置
  FVector Offset = RightVector.RotateAngleAxis(RandomAngle, FVector::UpVector) *
                   RandomDistance;
  FVector DispersedTarget = TargetLocation + Offset;

  // 使用分散后的位置作为移动目标，让士兵围绕目标形成包围圈
  EPathFollowingRequestResult::Type Result =
      AIController->MoveToLocation(DispersedTarget, AcceptanceRadius,
                                   true, // bStopOnOverlap
...
bool UXBSoldierBehaviorInterface::IsAtFormationPosition() const {
  ...
  float Distance =
      FVector::Dist2D(Soldier->GetActorLocation(), FormationPosition);
 ...
 * @param  无
 * @return 是否脱离战斗
 * @note   详细流程分析: 先处理“目标为跟随态”超距脱战 -> 距离超限强制脱战 ->
 * 目标非战斗时仅保留距离限制 -> 否则按无敌人时间判定 性能/架构注意事项:
 * 通过快速距离判断避免频繁感知查询，降低每帧开销
 */
bool UXBSoldierBehaviorInterface::ShouldDisengage() const {
  ...
  // 🔧 修复 BUG2 - 主将进入草丛时，士兵立即脱战并跟随
  // 说明：这是最高优先级的脱战条件，确保士兵不会在主将隐身时继续战斗
  if (AXBCharacterBase *Leader = Soldier->GetLeaderCharacter()) {
    if (Leader->IsHiddenInBush()) {
      UE_LOG(LogXBAI, Log, TEXT("士兵 %s 因主将进入草丛而脱战"),
             *Soldier->GetName());
      return true;
    }
  }
  ...
  if (AActor *CurrentTarget = Soldier->CurrentAttackTarget.Get()) {
    ...
    if (AXBSoldierCharacter *TargetSoldier =
            Cast<AXBSoldierCharacter>(CurrentTarget)) {
      bIsTargetInCombat =
          (TargetSoldier->GetSoldierState() == EXBSoldierState::Combat);
      bIsTargetFollowing =
          (TargetSoldier->GetSoldierState() == EXBSoldierState::Following);
    }
    // 目标是将领：检查其战斗状态
    else if (AXBCharacterBase *TargetLeader =
                 Cast<AXBCharacterBase>(CurrentTarget)) {
  ...
  // 说明：即使感知系统判定"无敌人"，只要 CurrentAttackTarget
  // 有效且存活，士兵就应继续战斗 这避免了战斗中因感知缓存刷新不及时而误触发脱战
  if (AActor *Target = Soldier->CurrentAttackTarget.Get()) {
    bool bTargetAlive = false;
    if (AXBSoldierCharacter *TargetSoldier =
            Cast<AXBSoldierCharacter>(Target)) {
      bTargetAlive =
          (TargetSoldier->GetSoldierState() != EXBSoldierState::Dead);
    } else if (AXBCharacterBase *TargetLeader =
                   Cast<AXBCharacterBase>(Target)) {
...
void UXBSoldierBehaviorInterface::FaceTarget(AActor *Target, float DeltaTime) {
  ...
  FVector Direction = (Target->GetActorLocation() - Soldier->GetActorLocation())
                          .GetSafeNormal2D();
