...
void UXBSoldierFollowComponent::BeginPlay()
{
    ...
    if (AActor* Owner = GetOwner())
    {
        ...
        SmoothedTargetPosition = LastFrameLocation;
    }
...
void UXBSoldierFollowComponent::UpdateLockedMode(float DeltaTime)
{
    ...
    // 🔧 修改 - 蛇形尾随：对目标位置进行插值，旋转时更平滑
    FVector DesiredTarget = TargetPosition;
    if (bEnableSnakeFollow)
    {
        SmoothedTargetPosition = FMath::VInterpTo(SmoothedTargetPosition, TargetPosition, DeltaTime, SnakeFollowInterpSpeed);
        DesiredTarget = SmoothedTargetPosition;
    }
    else
    {
    ...
    MoveTowardsTargetXY(DesiredTarget, DeltaTime, LockedFollowMoveSpeed, true);
...
void UXBSoldierFollowComponent::ExitCombatMode()
{
    ...
    SetFollowMode(EXBFollowMode::Locked);
    SmoothedTargetPosition = GetOwner() ? GetOwner()->GetActorLocation() : FVector::ZeroVector;
}
...
void UXBSoldierFollowComponent::TeleportToFormationPosition()
{
    ...
    SmoothedTargetPosition = Owner->GetActorLocation();
