...
void AXBSoldierCharacter::EnterCombat() {
  ...
  if (UCapsuleComponent *Capsule = GetCapsuleComponent()) {
    ...
    UE_LOG(LogXBCombat, Verbose, TEXT("士兵 %s 进入战斗，开启士兵间碰撞阻挡"),
           *GetName());
  }
...
void AXBSoldierCharacter::ExitCombat() {
  ...
  if (UCapsuleComponent *Capsule = GetCapsuleComponent()) {
    ...
    UE_LOG(LogXBCombat, Verbose, TEXT("士兵 %s 退出战斗，恢复士兵间碰撞重叠"),
           *GetName());
  }
...
/**
 * @brief 接收主将战斗指令
 * @param bEnterCombat 是否进入战斗
 * @param SuggestedTarget 建议攻击目标
 */
void AXBSoldierCharacter::ReceiveCombatOrder(bool bEnterCombat,
                                             AActor *SuggestedTarget) {
  if (bEnterCombat) {
    // 如果有建议目标，先设置目标
    if (SuggestedTarget && IsValid(SuggestedTarget)) {
      CurrentAttackTarget = SuggestedTarget;

      // 更新行为树黑板
      if (AXBSoldierAIController *AIController =
              Cast<AXBSoldierAIController>(GetController())) {
        AIController->SetTargetActor(SuggestedTarget);
      }
    }
 ...
 * @brief 从主将目标池领取目标
 * @return 领取到的目标
 * @note 暂时简单实现，后续对接 CommanderComponent
 */
AActor *AXBSoldierCharacter::ClaimTargetFromLeader() {
  // 如果已有有效目标，直接返回
