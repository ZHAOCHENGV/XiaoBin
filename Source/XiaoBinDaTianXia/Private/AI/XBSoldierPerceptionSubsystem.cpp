 ...
 */
bool UXBSoldierPerceptionSubsystem::QueryNearestEnemyByFaction(
    AActor* Querier,
    const FVector& Location,
    float Radius,
    EXBFaction TargetFaction,
    FXBPerceptionResult& OutResult,
    bool bPreferSoldiers)
{
    // 🔧 修改 - 查询者为空时直接失败，避免空指针
    if (!Querier || !IsValid(Querier))
    {
        return false;
    }

    // 🔧 修改 - 解析查询者阵营，避免将友军当作敌人
    EXBFaction QuerierFaction = EXBFaction::Neutral;
    if (AXBSoldierCharacter* QuerierSoldier = Cast<AXBSoldierCharacter>(Querier))
    {
        QuerierFaction = QuerierSoldier->GetFaction();
    }
    else if (AXBCharacterBase* QuerierLeader = Cast<AXBCharacterBase>(Querier))
    {
        QuerierFaction = QuerierLeader->GetFaction();
    }

    if (!UXBBlueprintFunctionLibrary::AreFactionsHostile(QuerierFaction, TargetFaction))
    {
        return false;
    }

    // 🔧 修改 - 初始化输出结果，避免历史脏数据干扰
    OutResult = FXBPerceptionResult();
    OutResult.ResultTime = FPlatformTime::Seconds();
    OutResult.bIsValid = true;

    const TArray<TWeakObjectPtr<AActor>>* FactionActors = RegisteredActors.Find(TargetFaction);
    if (!FactionActors)
    {
        return false;
    }
    ...
    const FVector QuerierLocation = Location;
    ...
    for (const TWeakObjectPtr<AActor>& WeakActor : *FactionActors)
    {
        AActor* Actor = WeakActor.Get();
        if (!Actor || !IsValid(Actor) || Actor == Querier)
        {
        ...
        else if (AXBCharacterBase* Leader = Cast<AXBCharacterBase>(Actor))
        {
            if (Leader->IsDead())
            {
                continue;
        ...
        const float DistSq = FVector::DistSquared(QuerierLocation, Actor->GetActorLocation());
        if (DistSq > RadiusSq)
        {
            continue;
        }

        OutResult.DetectedEnemies.Add(Actor);
    ...
    // 🔧 修改 - 按优先级返回最近目标
    if (bPreferSoldiers && NearestSoldier)
    {
        OutResult.NearestEnemy = NearestSoldier;
        OutResult.DistanceToNearest = FMath::Sqrt(NearestSoldierDistSq);
        return true;
    }
    ...
    if (NearestLeader)
    {
        OutResult.NearestEnemy = NearestLeader;
        OutResult.DistanceToNearest = FMath::Sqrt(NearestLeaderDistSq);
        return true;
    }
