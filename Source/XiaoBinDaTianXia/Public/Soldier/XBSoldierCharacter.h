...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnSoldierStateChanged,
                                             EXBSoldierState, OldState,
                                             EXBSoldierState, NewState);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSoldierDied,
                                            AXBSoldierCharacter *, Soldier);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnSoldierRecruited,
                                             AXBSoldierCharacter *, Soldier,
                                             AActor *, Leader);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDormantStateChanged,
                                             AXBSoldierCharacter *, Soldier,
                                             bool, bIsDormant);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnDropLandingComplete,
                                            AXBSoldierCharacter *, Soldier);
...
UCLASS()
class XIAOBINDATIANXIA_API AXBSoldierCharacter
    : public ACharacter,
...
public:
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier",
            meta = (DisplayName = "完整初始化"))
  void FullInitialize(UDataTable *DataTable, FName RowName,
                      EXBFaction InFaction);
   ...
   */
  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "获取发射物配置"))
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|Drop",
            meta = (DisplayName = "开始掉落飞行"))
  void StartDropFlight(const FVector &StartLocation,
                       const FVector &TargetLocation,
                       const FXBDropArcConfig &ArcConfig,
                       AXBCharacterBase *TargetLeader = nullptr);

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Drop",
            meta = (DisplayName = "是否掉落中"))
  bool IsDropping() const { return CurrentState == EXBSoldierState::Dropping; }

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Drop",
            meta = (DisplayName = "获取掉落进度"))
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|Drop",
            meta = (DisplayName = "绘制掉落抛物线"))
  void DrawDropDebugArc(float DurationOverride = -1.0f,
                        int32 SegmentOverride = -1) const;

  // ==================== 配置属性访问方法 ====================

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Data",
            meta = (DisplayName = "获取士兵类型"))
  EXBSoldierType GetSoldierType() const { return SoldierType; }

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Data",
            meta = (DisplayName = "获取显示名称"))
  FText GetDisplayName() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Data",
            meta = (DisplayName = "获取士兵标签"))
  FGameplayTagContainer GetSoldierTags() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "获取最大血量"))
  float GetMaxHealth() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "获取基础伤害"))
   ...
   */
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|Config",
            meta = (DisplayName = "应用运行时配置"))
  void ApplyRuntimeConfig(const struct FXBGameConfigData &GameConfig);

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "获取攻击范围"))
  float GetAttackRange() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "获取攻击间隔"))
  float GetAttackInterval() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Movement",
            meta = (DisplayName = "获取移动速度"))
  float GetMoveSpeed() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Movement",
            meta = (DisplayName = "获取冲刺倍率"))
  float GetSprintSpeedMultiplier() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Movement",
            meta = (DisplayName = "获取跟随插值速度"))
  float GetFollowInterpSpeed() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Movement",
            meta = (DisplayName = "获取旋转速度"))
  float GetRotationSpeed() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取视野范围"))
  float GetVisionRange() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取脱离距离"))
  float GetDisengageDistance() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取返回延迟"))
  float GetReturnDelay() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取到达阈值"))
  float GetArrivalThreshold() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取避让半径"))
  float GetAvoidanceRadius() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取避让权重"))
  ...
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier",
            meta = (DisplayName = "进入战斗"))
  void EnterCombat();

  UFUNCTION(BlueprintCallable, Category = "XB|Soldier",
            meta = (DisplayName = "退出战斗"))
  void ExitCombat();
   ...
   * 注意事项: 死亡/休眠/掉落状态不处理
   */
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|Combat",
            meta = (DisplayName = "接收分配目标"))
...
public:
  ...
  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|AI",
            meta = (DisplayName = "范围内有敌人"))
  bool HasEnemiesInRadius(float Radius) const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取到目标距离"))
  float GetDistanceToTarget(AActor *Target) const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "是否在攻击范围内"))
  bool IsInAttackRange(AActor *Target) const;

  UFUNCTION(BlueprintCallable, Category = "XB|Soldier",
            meta = (DisplayName = "返回队列"))
  void ReturnToFormation();

  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|AI",
            meta = (DisplayName = "移动到编队位置"))
  void MoveToFormationPosition();

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取编队位置"))
  FVector GetFormationWorldPosition() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "获取编队位置(安全)"))
  FVector GetFormationWorldPositionSafe() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "是否到达编队位置"))
  bool IsAtFormationPosition() const;

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|AI",
            meta = (DisplayName = "是否到达编队位置(安全)"))
  bool IsAtFormationPositionSafe() const;

  // ==================== 逃跑系统 ====================

  UFUNCTION(BlueprintCallable, Category = "XB|Soldier",
            meta = (DisplayName = "设置逃跑状态"))
  void SetEscaping(bool bEscaping);

  UFUNCTION(BlueprintPure, Category = "XB|Soldier",
            meta = (DisplayName = "是否正在逃跑"))
  bool IsEscaping() const { return bIsEscaping; }

  // ==================== 对象池支持 ====================

  UFUNCTION(BlueprintCallable, Category = "XB|Soldier|Pool",
            meta = (DisplayName = "重置状态"))
  void ResetForPooling();

  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Pool",
            meta = (DisplayName = "是否池化士兵"))
  ...
  UPROPERTY(BlueprintAssignable, Category = "XB|Soldier|Dormant",
            meta = (DisplayName = "休眠状态变化"))
  FOnDormantStateChanged OnDormantStateChanged;

  UPROPERTY(BlueprintAssignable, Category = "XB|Soldier|Drop",
            meta = (DisplayName = "掉落完成"))
  FOnDropLandingComplete OnDropLandingComplete;

  // ==================== 组件访问 ====================

  UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "组件",
            meta = (DisplayName = "跟随组件"))
  TObjectPtr<UXBSoldierFollowComponent> FollowComponent;

  UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "组件",
            meta = (DisplayName = "调试组件"))
 ...
 // 🔧 新增 - 休眠无敌状态控制
  ...
  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Behavior",
            meta = (DisplayName = "获取行为接口"))
  UXBSoldierBehaviorInterface *GetBehaviorInterface() const {
   ...
   *       其他情况返回0，避免过渡动画异常
   */
  UFUNCTION(BlueprintPure, Category = "XB|Soldier|Animation",
            meta = (DisplayName = "获取动画移动速度"))
   ...
   *         性能/架构注意事项: 仅在状态变化时执行
   */
  UFUNCTION(BlueprintCallable, Category = "草丛",
            meta = (DisplayName = "设置草丛隐身"))
   ...
   * @return 是否隐身
   */
  UFUNCTION(BlueprintPure, Category = "草丛",
   ...
   * @return 无
   * @note   详细流程分析:
   * 统一走内部跟随配置入口，确保移动组件/编队事件/AI延迟启动一致
   *         性能/架构注意事项: 仅在主将数据初始化完成时触发，避免频繁调用
   */
  void RefreshFollowingAfterLeaderInit(AXBCharacterBase *Leader,
                                       int32 SlotIndex);
...
protected:
  /**
   * @brief  刷新近战命中GA配置
   * @note   详细流程分析: 读取数据表普攻配置 -> 覆盖近战GA ->
   * 初始化ASC并授予能力 性能/架构注意事项:
   * 仅在初始化阶段调用，避免运行时重复授予
   */
...
protected:
  ...
  UPROPERTY(BlueprintReadOnly, Category = "状态",
            meta = (DisplayName = "当前状态"))
  EXBSoldierState CurrentState = EXBSoldierState::Idle;

  UPROPERTY(BlueprintReadOnly, Category = "状态",
            meta = (DisplayName = "跟随目标"))
  TWeakObjectPtr<AActor> FollowTarget;

  UPROPERTY(BlueprintReadOnly, Category = "状态",
            meta = (DisplayName = "槽位索引"))
  ...
  UPROPERTY(BlueprintReadOnly, Category = "配置",
            meta = (DisplayName = "士兵血量覆盖值"))
  float CachedHealthOverride = 0.0f;

  UPROPERTY(BlueprintReadOnly, Category = "状态",
            meta = (DisplayName = "正在逃跑"))
  ...
  // ✨ 新增 - 队形尾随配置
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "XB|Soldier|Formation",
            meta = (DisplayName = "启用编队尾随插值"))
  bool bEnableFormationTailDelay = true;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "XB|Soldier|Formation",
            meta = (DisplayName = "尾随延迟/槽位", ClampMin = "0.0"))
  float FormationTailDelayPerSlot = 0.05f;

  // ==================== AI配置 ====================

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AI",
            meta = (DisplayName = "行为树"))
  TObjectPtr<UBehaviorTree> BehaviorTreeAsset;

  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AI",
            meta = (DisplayName = "AI控制器类"))
   ...
   *         🔧 修改 - 从 0.25s 降低到 0.05s，减少士兵响应延迟
   */
  UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AI",
            meta = (DisplayName = "自动寻敌检查间隔", ClampMin = "0.05"))
  float AutoEngageCheckInterval = 0.05f;
  ...
  FVector ComputeGroundSnappedLocation(const FVector &DesiredLocation,
                                       const FXBDropArcConfig &ArcConfig) const;
