...
struct FXBSpawnableActorEntry
{
	...
	/** 显示名称（中文，用于UI） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "放置配置", meta = (DisplayName = "显示名称"))
	FText DisplayName;

	/** Actor 类（蓝图或 C++ 类） */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "放置配置", meta = (DisplayName = "Actor类"))
	...
	/** 默认缩放 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "放置配置", meta = (DisplayName = "默认缩放"))
	FVector DefaultScale = FVector::OneVector;

	/** 默认旋转偏移 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "放置配置", meta = (DisplayName = "默认旋转"))
